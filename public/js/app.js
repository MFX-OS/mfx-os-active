...
<div style="display:flex;flex-wrap:wrap;gap:14px;align-items:center;margin:8px 0;padding:8px 10px;background:var(--bg);border:1px solid var(--bdr);border-radius:6px;font-size:11px;font-weight:600;color:var(--tx)">
  <label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" data-field="pouchZipper" ${f.pouchZipper?'checked':''} onchange="asave()" style="margin:0">Zipper</label>
  <label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" data-field="pouchCRZipper" ${f.pouchCRZipper?'checked':''} onchange="asave()" style="margin:0">CR Zipper</label>
  <label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" data-field="pouchGusset" ${f.pouchGusset?'checked':''} onchange="asave()" style="margin:0">Gusset <span style="font-weight:400;color:var(--tx3);font-size:10px">(forces SUP)</span></label>
</div>
...
<!-- 2026-06-01 round 67: Display Box + Master Shipper configurator.
     Ported from "Display & Master Confi. (2)" sheet of the same
     Excel workbook. The dimensions are derived from the pouch width
     and the stacking pattern; $/display and $/master come from your
     carton supplier (manual entry). Engine adds qty×$/display +
     qty×$/master to every pricing tier when both are filled. -->
<div class="vsec">📦 Display Box & Master Shipper <span style="font-size:8px;color:var(--tx3)">Cartoning — pouches per display × stacking pattern × supplier cost</span></div>
<div class="var-row"><span class="var-lbl">Pouches/Display</span><input class="var-inp hi" data-field="pouchesPerDisplay" type="number" step="1" value="${f.pouchesPerDisplay||''}" placeholder="0 = no display" oninput="asave()"><span class="var-unit">#</span></div>
<div class="var-row"><span class="var-lbl">Master Pattern</span><div style="display:flex;gap:6px;flex:1"><input data-field="cartonCols" type="number" step="1" value="${f.cartonCols||1}" oninput="asave()" title="Cols" style="width:60px;padding:4px 6px;font-size:11px;background:var(--bg);border:1px solid var(--bdr);border-radius:4px;color:var(--tx);text-align:center"><span style="color:var(--tx3);align-self:center;font-size:11px">×</span><input data-field="cartonRows" type="number" step="1" value="${f.cartonRows||1}" oninput="asave()" title="Rows" style="width:60px;padding:4px 6px;font-size:11px;background:var(--bg);border:1px solid var(--bdr);border-radius:4px;color:var(--tx);text-align:center"><span style="color:var(--tx3);align-self:center;font-size:11px">×</span><input data-field="cartonStacks" type="number" step="1" value="${f.cartonStacks||1}" oninput="asave()" title="Stacks" style="width:60px;padding:4px 6px;font-size:11px;background:var(--bg);border:1px solid var(--bdr);border-radius:4px;color:var(--tx);text-align:center"></div><span class="var-unit">C×R×S</span></div>
<div class="var-row"><span class="var-lbl">$/Display</span><input class="var-inp" data-field="displayCostEa" type="number" step="0.01" value="${f.displayCostEa||''}" placeholder="supplier $/unit" oninput="asave()"><span class="var-unit">$</span></div>
<div class="var-row"><span class="var-lbl">$/Master</span><input class="var-inp" data-field="masterCostEa" type="number" step="0.01" value="${f.masterCostEa||''}" placeholder="supplier $/unit" oninput="asave()"><span class="var-unit">$</span></div>
<details style="margin:6px 0;font-size:10px"><summary style="cursor:pointer;color:var(--tx2);padding:4px 0">Carton spec details (for supplier PO)</summary>
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-top:6px;padding:8px;background:var(--bg);border-radius:4px">
    <div class="fg" style="margin:0"><label style="font-size:9px">Display Material</label><select data-field="displayMaterial" onchange="asave()" style="width:100%;padding:6px;font-size:11px;background:var(--bg2);border:1px solid var(--bdr);border-radius:4px;color:var(--tx)"><option value=""></option>${['CCNB','SBS','CNK','SUS','TTAB','12 pt','14 pt','16 pt','18 pt','20 pt','24 pt'].map(function(m){return '<option value="'+m+'"'+(f.displayMaterial===m?' selected':'')+'>'+m+'</option>'}).join('')}</select></div>
    <div class="fg" style="margin:0"><label style="font-size:9px">Display Coating</label><select data-field="displayCoating" onchange="asave()" style="width:100%;padding:6px;font-size:11px;background:var(--bg2);border:1px solid var(--bdr);border-radius:4px;color:var(--tx)"><option value=""></option>${['AQ Gloss','UV Varnish','Matte','None'].map(function(c){return '<option value="'+c+'"'+(f.displayCoating===c?' selected':'')+'>'+c+'</option>'}).join('')}</select></div>
    <div class="fg" style="margin:0"><label style="font-size:9px">Display Style</label><select data-field="displayStyle" onchange="asave()" style="width:100%;padding:6px;font-size:11px;background:var(--bg2);border:1px solid var(--bdr);border-radius:4px;color:var(--tx)"><option value=""></option>${['RSC','ECT','PLAIN','Printed 1 Color','Kraft','Flute'].map(function(s){return '<option value="'+s+'"'+(f.displayStyle===s?' selected':'')+'>'+s+'</option>'}).join('')}</select></div>
    <div class="fg" style="margin:0"><label style="font-size:9px">Display Colors</label><select data-field="displayColors" onchange="asave()" style="width:100%;padding:6px;font-size:11px;background:var(--bg2);border:1px solid var(--bdr);border-radius:4px;color:var(--tx)"><option value=""></option>${['4CP','1 PMS','2 PMS','3 PMS','4 PMS','5 PMS','6 PMS'].map(function(c){return '<option value="'+c+'"'+(f.displayColors===c?' selected':'')+'>'+c+'</option>'}).join('')}</select></div>
    <div class="fg" style="margin:0;grid-column:1/-1"><label style="font-size:9px">Master Material / Style</label><div style="display:grid;grid-template-columns:1fr 1fr;gap:6px"><input data-field="masterMaterial" value="${esc(f.masterMaterial||'')}" placeholder="e.g. 200# RSC" oninput="asave()" style="padding:6px;font-size:11px;background:var(--bg2);border:1px solid var(--bdr);border-radius:4px;color:var(--tx)"><input data-field="masterStyle" value="${esc(f.masterStyle||'')}" placeholder="e.g. ECT-32" oninput="asave()" style="padding:6px;font-size:11px;background:var(--bg2);border:1px solid var(--bdr);border-radius:4px;color:var(--tx)"></div></div>
  </div>
</details>
<div id="cartonPreview" style="font-size:10px;color:var(--tx3);line-height:1.5;padding:6px 8px;background:var(--bg);border-radius:4px;border:1px dashed var(--bdr);margin-bottom:10px"></div>
...
<div style="display:flex;flex-wrap:wrap;gap:14px;align-items:center;padding:8px 10px;background:var(--bg);border:1px solid var(--bdr);border-radius:6px;font-size:11px;font-weight:600;color:var(--tx)">
<label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" data-field="showSetupOnPDF" ${f.showSetupOnPDF?'checked':''} onchange="asave()" style="margin:0">Setup costs</label>
<label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" data-field="showShippingOnPDF" ${f.showShippingOnPDF?'checked':''} onchange="asave()" style="margin:0">Shipping</label>
<label style="display:inline-flex;align-items:center;gap:6px;cursor:pointer"><input type="checkbox" data-field="showPlateOnPDF" ${f.showPlateOnPDF?'checked':''} onchange="asave()" style="margin:0">Plate / die</label>
</div>
...
_paneIds.forEach(function(pid,i){var pane=$(pid);if(!pane)return;var nav=document.createElement('div');nav.style.cssText='display:flex;gap:8px;margin-top:16px;padding:12px 0;border-top:1px solid var(--bdr)';// 2026-05-27: defensive ||'…' fallback so null/missing _tabNames entries never produce empty/literal-"null" button labels.
if(i>0){var prev=document.createElement('button');prev.className='btn btn-ghost btn-sm';prev.style.cssText='flex:1;display:flex;align-items:center;justify-content:center;gap:6px';prev.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="15 18 9 12 15 6"/></svg> '+(_tabNames[i-1]||'Back');prev.onclick=function(){switchET(i-1)};nav.appendChild(prev)}if(i<_paneIds.length-1){var next=document.createElement('button');next.className='btn btn-pr btn-sm';next.style.cssText='flex:1;display:flex;align-items:center;justify-content:center;gap:6px;margin-left:auto';next.innerHTML=(_tabNames[i+1]||'Next')+' <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="9 18 15 12 9 6"/></svg>';next.onclick=function(){switchET(i+1)};nav.appendChild(next)}pane.appendChild(nav)})}
