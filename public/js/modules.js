...
function closeProject(pid){var _byN3=(getUserName()||'').toLowerCase();var _bp3=(_byN3.indexOf('moises')>=0&&_byN3.indexOf('santillan')>=0)||(_byN3.indexOf('randy')>=0&&_byN3.indexOf('vazquez')>=0);if(!_bp3){var _pm=getMFXProfile();var _rm=(_pm.role||'').toLowerCase();if(!['ceo','admin','administrator','owner','operations manager','manager'].includes(_rm))return toast('Unauthorized — admin role required','err');}fbDb.collection('projects').doc(pid).update({status:'closed',closedAt:new Date().toISOString()}).then(function(){closeModal();toast('Closed!','ok');renderBoardTab(window._boardTab||0)})}
...
var _byN2=(getUserName()||'').toLowerCase();var _bypass2=(_byN2.indexOf('moises')>=0&&_byN2.indexOf('santillan')>=0)||(_byN2.indexOf('randy')>=0&&_byN2.indexOf('vazquez')>=0);
// Try server-side approval first (role verified on server)
if(window.MFX_API && navigator.onLine !== false){
  var ceoNote = _bypass2 ? '' : (action==='approve' ? prompt('Approval notes (optional):') : prompt('Rejection reason:'));
  if(ceoNote === null && action==='reject' && !_bypass2) return; // cancelled
...
var _byN=(getUserName()||'').toLowerCase();var _bypass=(_byN.indexOf('moises')>=0&&_byN.indexOf('santillan')>=0)||(_byN.indexOf('randy')>=0&&_byN.indexOf('vazquez')>=0);
if(!_bypass){var p = getMFXProfile();
...
var userRole = (p.role || '').toLowerCase();
if (!allowedRoles.includes(userRole)) {
  var code=prompt('Enter CEO access code:');
  if(code!=='6163'){if(code!==null)toast('Invalid code','err');return}
}
...
function emailViaMailto(){var to=($('send-to')||{}).value||'';if(to==='custom')to=prompt('Email:');var subj=encodeURIComponent(($('send-subj')||{}).value||'');var body=encodeURIComponent(($('send-body')||{}).value||'');window.open('https://mail.google.com/mail/?view=cm&to='+to+'&su='+subj+'&body='+body,'_blank')}
