...
saveC(cs){const b=fbDb.batch();cs.forEach(c=>b.set(fbDb.collection('customers').doc(c.id),c));firestoreRetry(function(){ return b.commit(); }).catch(function(e){ console.error('saveC:',e); if(typeof toast==='function') toast('Save failed — check connection','err'); });_cache.customers=cs},
...
// PERF-01/02 fix (2026-05-24): Bounded listeners.
// Limits prevent unbounded Firestore reads at scale. Older docs accessible via
// direct query (openEditor uses doc().get()) so editing pre-window quotes still
// works — the editId preservation block below also keeps the active edit visible.
// Tune these constants if the working set grows beyond the limits.
...
_coreListeners.push(fbDb.collection('quotes').orderBy('updatedAt','desc').limit(QUOTES_LISTENER_LIMIT).onSnapshot(s=>{
  ...
  // Preserve the actively-edited quote if the snapshot doesn't include it yet (pending write OR out-of-window)
  ...
  if(_ready){
    ...
    else if(S.etab===14&&typeof renderWorkflow==='function'){renderWorkflow();if(typeof renderConnections==='function')renderConnections()}
  }
...
// Note: All dept-*-home aliases (both short and long form) are baked into every
// dept's list so the hamburger menu's cross-dept landing pages always work.
// Short names (dept-prod-home, dept-qa-home, etc.) come from full-menu.js; long
// names (dept-production-home, dept-quality-home, etc.) are legacy. Both work.
var ALL_DEPT_HOMES=[
...
var DEPT_ALLOWED_VIEWS={
  'client services':['quotes','customers','orders','templates','clientservices','sales','vendorpos','vendorprofile','ppd','jobtracker','dashboard','ceodash','launchpad','aiops',...ALL_DEPT_HOMES],
  'sales':['quotes','customers','orders','templates','clientservices','sales','vendorpos','vendorprofile','dashboard','launchpad',...ALL_DEPT_HOMES],
  'estimation':['quotes','customers','orders','templates','clientservices','sales','vendorpos','vendorprofile','ppd','dashboard','launchpad',...ALL_DEPT_HOMES],
  'pre-press':['ppd','jobtracker','production','dashboard','launchpad',...ALL_DEPT_HOMES],
  'prepress':['ppd','jobtracker','production','dashboard','launchpad',...ALL_DEPT_HOMES],
  'production':['production','jobtracker','operator','ppd','dashboard','launchpad',...ALL_DEPT_HOMES],
  'manufacturing':['production','jobtracker','operator','ppd','dashboard','launchpad',...ALL_DEPT_HOMES],
  'logistics':['logistics','production','jobtracker','dashboard','launchpad',...ALL_DEPT_HOMES],
  'shipping':['logistics','production','jobtracker','dashboard','launchpad',...ALL_DEPT_HOMES],
  'warehouse':['logistics','production','jobtracker','dashboard','launchpad',...ALL_DEPT_HOMES],
  'quality':['quality','capa','gmp','audit','training','doccontrol','sqfdatalogs','fsqms','records','dashboard','launchpad',...ALL_DEPT_HOMES],
  'qa':['quality','capa','gmp','audit','training','doccontrol','sqfdatalogs','fsqms','records','dashboard','launchpad',...ALL_DEPT_HOMES],
  'finance':['accounting','finance','dashboard','launchpad',...ALL_DEPT_HOMES],
  'accounting':['accounting','finance','dashboard','launchpad',...ALL_DEPT_HOMES],
  'fsqms':['gmp','capa','audit','training','doccontrol','sqfdatalogs','fsqms','records','quality','dashboard','launchpad',...ALL_DEPT_HOMES],
  'sqf':['gmp','capa','audit','training','doccontrol','sqfdatalogs','fsqms','records','quality','dashboard','launchpad',...ALL_DEPT_HOMES],
  'food safety':['gmp','capa','audit','training','doccontrol','sqfdatalogs','fsqms','records','quality','dashboard','launchpad',...ALL_DEPT_HOMES],
...
async function checkAndSeedData(){const snap=await fbDb.collection('customers').limit(1).get();
...
function _startSpecOverlayListeners(){
  ...
  ['dies','films','labels','varnishes'].forEach(function(type){
    _coreListeners.push(fbDb.collection('materialsCatalog').doc(type).onSnapshot(function(doc){
      ...
      // If the spec manager modal is open for this type, re-render so the user sees teammates' changes live
      try{if(document.querySelector('.spec-row')&&typeof showSpecManager==='function'){/* leave as-is; user is interacting */}}catch(e){}
...
async function genQNServer(){if(window.fbDb){try{return await genQNFirestore()}catch(e){console.error('genQNServer firestore fail:',e)}}if(typeof requestServerNumber==='function'){try{return await requestServerNumber('quote',function(){return genQN()})}catch(e){}}return genQN()}
...
// Get Firestore atomic number FIRST, then save
...
if(asRev){n.rev=nextRev(o.rev);_finishDup()}else{n.rev='A';n.parentQuoteId=null;n.quoteNum=genQN();genQNFirestore().then(function(sn){n.quoteNum=sn}).catch(function(e){console.error('Dup quote number failed:',e);toast('Using temporary quote number','warn')}).then(_finishDup)}}
...
const oldFields=JSON.stringify(q.fields);
...
function saveCust(c){const all=DB.customers();const i=all.findIndex(x=>x.id===c.id);if(i>=0)all[i]=c;else all.unshift(c);DB.saveC(all);toast('Saved','ok');if(S.view==='profile'&&S.profileId===c.id)renderProfile(c.id);else if(S.view==='customers')renderCust();
